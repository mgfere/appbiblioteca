...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $query = "SELECT nombre, correo FROM administradores WHERE matricula = '$matricula'";
        ...
        if (mysqli_num_rows($resultado) > 0) {
            ...
            if (empty($admin['correo'])) {
                $errores[] = "Este administrador no tiene un correo electrónico registrado.";
            } else {
                $token = substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyz"), 0, 8);
                $expiracion = date('Y-m-d H:i:s', strtotime('+15 minutes'));

                $query_update = "UPDATE administradores SET token = '$token', token_expira = '$expiracion' WHERE matricula = '$matricula'";
                ...
                try {
                    ...
                    header('Location: verificar-token.php');
                    exit;
