...
<div class="container main--content">
    <div class="header--wrapper">
        <div class="header--title">
          ...
          echo 'Administrador';
            ...
            <h2>Escanear Libro para Préstamo Presencial Usuario-Externo</h2>
        </div>
        <div class="user--info">
    ...
    <div class="tabular--wrapper">
        ...
        <div class="btn-volver-wrapper">
            <a href="panel-usuarios-externos.php" class="btn-volver">
                <i class="fas fa-arrow-left"></i> Volver
            </a>
        </div>
        <div class="scanner-interface">
            <h2>ESCANEAR CÓDIGO DEL LIBRO</h2>
            <div class="manual-input" style="display: flex; flex-direction: column; align-items: center; width: 100%;">
                Escanea o ingresa el ID del libro
                <div class="input-select-wrapper" style="width: 100%; display: flex; flex-direction: column; align-items: center;">
                    <input type="text" id="bookIdInput" placeholder="ID del libro" style="margin-bottom: 8px;" autofocus />
                </div>
            </div>
            <button class="btn-continuar" id="btnContinuarPrestamo">Continuar</button>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    // <-- CAMBIO 1: Guardamos el ID del usuario que viene de PHP en una variable de JavaScript.
    const userId = <?php echo json_encode($idUsuario); ?>;
            ...
            success: function(response) {
                if (response.success) {
                    if (response.action === 'redirect') {
                        // <-- CAMBIO 2: Añadimos el ID del usuario a la URL de redirección.
                        window.location.href = `prestamos/registrar-prestamo-usuario-EXTRegistrado.php?id=${userId}&bookId=${response.bookId}`;
    ...
    const displayBookSelector = (libros) => {
        ...
        scannerInterface.appendChild(container);

        btnContinuar.onclick = () => {
            const selectedId = select.value;
            if (selectedId) {
                // <-- CAMBIO 3: También añadimos el ID del usuario aquí.
                window.location.href = `prestamos/registrar-prestamo-usuario-EXTRegistrado.php?id=${userId}&bookId=${selectedId}`;
            } else {
