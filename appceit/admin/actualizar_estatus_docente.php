...
// --- Obtener la matrícula del docente desde SQL Server ---
$query_matricula = "SELECT Matricula FROM [GestionUsuarios].[dbo].[Docentes] WHERE IdDocente = ?";
$params_matricula = [$id];
$stmt_matricula = sqlsrv_query($conn_sqlsrv, $query_matricula, $params_matricula);

if ($stmt_matricula === false) {
    responder_json(false, 'Error al obtener datos del docente.', 500);
}

$docente = sqlsrv_fetch_array($stmt_matricula, SQLSRV_FETCH_ASSOC);
sqlsrv_free_stmt($stmt_matricula);

if (!$docente) {
    responder_json(false, 'Docente no encontrado.', 404);
}

$matricula = $docente['Matricula'];
$tipoUsuario = 'docente';

// --- Verificar si el docente tiene préstamos activos ---
if ($estatus === 0) {
    ...
    if ($resultado_prestamos['total'] > 0) {
        responder_json(false, "Error: No se puede desactivar un docente con {$resultado_prestamos['total']} préstamo(s) activo(s).", 409);
    }
}
...
if ($resultado) {
    ...
    responder_json(true, "Docente {$accion} correctamente.");
} else {
    responder_json(false, 'Error al actualizar el estatus del docente.', 500);
}
